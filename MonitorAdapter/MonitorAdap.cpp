...
int CALLBACK MonitorAdapter::MonitorEnumProc(HMONITOR hMonitor, HDC hdc,
...
// 得到所有显示器的名称
void MonitorAdapter::GetAllMonitorName(
    VEC_MONITORMODE_INFO& vecMonitorListInfo) {
  ...
  for (int i = 0; i < g_hMonitorGroup.size(); i++) {
    ...
    if (itBeg == itEnd) {
      ...
      _tcscpy_s(tmpMonitorInfo.szDevice, sizeof(tmpMonitorInfo.szDevice),
                mixTemp.szDevice);
...
// 得到所有显示器的模式
void MonitorAdapter::GetAllDisplayMode(
    VEC_MONITORMODE_INFO& vecMonitorListInfo) {
  ...
  for (NULL; itBeg != itEnd; ++itBeg) {
    ...
    do {
      ...
      if (bRetVal) {
        ...
        vector<MonitorModeInfo>::iterator itBeg_Mode =
            itBeg->m_vecModeInfo.begin();
        vector<MonitorModeInfo>::iterator itEnd_Mode =
            itBeg->m_vecModeInfo.end();
        for (NULL; itBeg_Mode != itEnd_Mode; ++itBeg_Mode) {
          // 如果已经在列表中找到,则结束本次循环
          if ((itBeg_Mode->m_nWidth == devmode.dmPelsWidth) &&
              (itBeg_Mode->m_nHeight == devmode.dmPelsHeight)) {
            bFind = true;
            break;
          }
        ...
        if (!bFind) {
          if (itBeg_Mode == itEnd_Mode) {
            itBeg->m_vecModeInfo.push_back(
                MonitorModeInfo(devmode.dmPelsWidth, devmode.dmPelsHeight));
          } else {
            itBeg->m_vecModeInfo.insert(
                itBeg_Mode,
                MonitorModeInfo(devmode.dmPelsWidth, devmode.dmPelsHeight));
          }
        }
...
int MonitorAdapter::ChangMonitorReselotion(HMONITOR hMonitor, const int nWidth,
                                           const int nHight, const int nFre,
                                           const int nColorBits) {
  if (NULL == hMonitor) {
    return -1;
  }
  MONITORINFOEX mi;
  ...
  int nRet = ChangeDisplaySettingsEx(
      mi.szDevice, &DeviceMode, NULL,
      CDS_GLOBAL | CDS_NORESET | CDS_UPDATEREGISTRY, NULL);
  if (DISP_CHANGE_BADMODE == nRet) {
    ChangeDisplaySettingsEx(mi.szDevice, &DeviceMode, NULL,
...
void MonitorAdapter::GetCurrentReselotion(int& nWidth, int& nHeight, int& nFreq,
                                          int& nBits) {
  DEVMODE DeviceMode;
...
void MonitorAdapter::GetCurrentReselotion(LPCWSTR lpszDeviceName, int& nWidth,
                                          int& nHeight, int& nFreq,
                                          int& nBits) {
  DEVMODE DeviceMode;
